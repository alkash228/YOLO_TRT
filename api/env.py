...
def load_settings_from_env() -> PipelineSettings:
    ...
    settings = PipelineSettings(
        ...
        realtime_mode=_env_bool("YOLO_DRT_REALTIME_MODE", True),
        gpu_full_batch=_env_bool("YOLO_DRT_GPU_FULL_BATCH", False),
        infer_batch_size=_env_int("YOLO_DRT_INFER_BATCH_SIZE", 64),
        gpu_queue_depth=_env_int("YOLO_DRT_GPU_QUEUE_DEPTH", 4),
        decode_prefetch=_env_int("YOLO_DRT_DECODE_PREFETCH", 4),
        track_buffer=_env_int("YOLO_DRT_TRACK_BUFFER", 300),
        smart_ram_budget=_env_bool("YOLO_DRT_SMART_RAM_BUDGET", True),
        max_process_ram_gb=_env_float("YOLO_DRT_MAX_PROCESS_RAM_GB", 10.0),
        ...
        frame_source_mode=os.environ.get("YOLO_DRT_FRAME_SOURCE_MODE", "auto").strip() or "auto",
        tensorrt_imgsz=_env_int("YOLO_DRT_TRT_IMGSZ", 640),
        tensorrt_max_batch=_env_int("YOLO_DRT_TRT_MAX_BATCH", 32),
        tensorrt_fp16=_env_bool("YOLO_DRT_TRT_FP16", True),
        tensorrt_workspace_gb=_env_float("YOLO_DRT_TRT_WORKSPACE_GB", 2.0),
        tensorrt_autocast_fast=_env_bool("YOLO_DRT_TRT_AUTOCAST_FAST", True),
